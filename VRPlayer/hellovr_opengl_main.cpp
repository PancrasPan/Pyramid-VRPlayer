...
TClient::TClient()
{
	pic_inbuff = new char[BUFF_SIZE+2];
	pic_outbuff = new char[PIC_SIZE];//??
}
TClient::~TClient()
{
	delete pic_inbuff;
...
//初始化socket操作
int TClient::client_transfer_init()
{
	SOCKADDR_IN servaddr;
	//初始化Window Socket
	if (WSAStartup(MAKEWORD(2, 2), &wsaData)) {
...
/*设置套接字为非阻塞模式*/
int TClient::set_non_block(SOCKET socket)
{
	/*标识符非0允许非阻塞模式*/
	int ret;
	unsigned long flag = 1;
...
/*非阻塞套接字的connect*/
int TClient::connect_non_block(SOCKET socket,const struct sockaddr *address, int address_len)
{
	int sel;
	struct timeval tm;
	if (connect(socket, address, address_len) < 0)
	{
		...
		sel = select(socket + 1, NULL, &wfd, NULL, &tm);
		if (sel <0)
		{
			printf("select socket error: (errno: %d)\n", WSAGetLastError());
			return -1;
...
/*非阻塞套接字的recv*/
int TClient::recv_non_block(SOCKET socket,int length, int flags)
{
	int recv_len, ret_val, sel;
	struct timeval tm;
	...
	for (recv_len = 0; recv_len < length;)
	{
		...
		if (sel < 0) {   //连接失败
			...
			if (FD_ISSET(socket, &read_fd)) { //如果真正可读
				ret_val = recv(socket, pic_inbuff + recv_len, length - recv_len, flags);
				if (ret_val < 0) {
					printf("recv error\n");
					return ret_val;
				}
...
/*非阻塞套接字的send*/
int TClient::send_non_block(SOCKET socket,int length, int flags)
{
	int send_len, ret_val, sel;
	struct timeval tm;
	...
	for (send_len = 0; send_len < length;)
	{
		...
		if (sel < 0) {   //连接失败
			...
			if (FD_ISSET(socket, &write_fd)) { //如果真正可写
				ret_val = send(socket, pic_inbuff + send_len, length - send_len, flags);
				if (ret_val < 0) {
					printf("send error\n");
					return -2;
				}
...
//销毁客户端传输socket
void TClient::client_transfer_destroy()
{
	dec.decoder_flush((uint8_t*)pic_outbuff);
	dec.decoder_destroy();
	closesocket(sockfd);   //客户端关闭连接
	WSACleanup();

}
//客户端执行函数
void TClient::client_excute()
{
	//初始化socket
	/*if (client_transfer_init() == -1) {
		printf("Socket error!\n");
		exit(1);
	}*/
	int   data_size;
	...
	while(1)
	{
		//gop_num = htonl(i);
		//angle = htons(0);	
		//memcpy(pic_inbuff, (char*)&angle, 2);
		////memcpy(pic_inbuff + 2, (char*)&gop_num, 4);
		//data_size=send_non_block(sockfd,2, 0);
		//cout << "send_size=" << data_size<<endl;
		//读入编码后的二进制码流
		data_size = recv_non_block(sockfd, 6, 0);
		
		if (data_size == 6)
		{
			QueryPerformanceCounter(&nBeginTime_1);
			memcpy(&len, pic_inbuff, 4);
			pre_viewpoint = cur_viewpoint;
			//first_flag = true;
			memcpy(&cur_viewpoint, pic_inbuff + 4, 2);
			while (1) {
				data_size = recv_non_block(sockfd, (len > BUFF_SIZE) ? BUFF_SIZE : len, 0);

				///cout << "recv_len=" << data_size << endl;
				if (data_size) {//使用parser将读入的数据解析为帧
					len -= data_size;

					//cout << "data_len=" << data_size << endl;
					dec.decoder_decode((uint8_t*)(pic_inbuff), (uint8_t*)pic_outbuff, data_size);
...
//			frame_fifo[Mbuff1].Mbuff = avframe_to_cvmat(frame);
//			frame_fifo[Mbuff1].viewpoint = cur_viewpoint;//sess.viewpoint;
//			//fprintf(stdout,"store viewpoint index %d\n", frame_fifo[Mbuff1].viewpoint);
...
void MDecode::decoder_flush(uint8_t *pic_outbuff)
{
	...
	ret = av_parser_parse2(parser, context, &packet->data, &packet->size, NULL, 0, AV_NOPTS_VALUE, AV_NOPTS_VALUE, 0);
	if (ret < 0) {
	...
	if (packet->size) {
		...
		while (ret >= 0) {
			...
			SDL_SemPost(Lock1);
			//QueryPerformanceCounter(&nEndTime_1);
			//cout << "after flush" << context->frame_number << " " << (double)(nEndTime_1.QuadPart - nBeginTime_1.QuadPart) / (double)nFreq.QuadPart * 1000 << "ms" << endl;
		...
		if (packet->size) { //一帧分割完成,解码一帧
			...
			while (ret >= 0) {
				...
				if ((Mbuff1 + 1) % MaxLoadFrameBuffSize != Mbuff2) {
					...
					//if (first_flag)
					//{
					//	frame_fifo[Mbuff1].viewpoint = pre_viewpoint;//zsy
					//	first_flag = false;
					//}
					//else
						frame_fifo[Mbuff1].viewpoint = cur_viewpoint;//sess.viewpoint;
...
#endif // DEBUG
				//QueryPerformanceCounter(&nEndTime_1);
				//cout <<"after decode"<<context->frame_number<<" " << (double)(nEndTime_1.QuadPart - nBeginTime_1.QuadPart) / (double)nFreq.QuadPart * 1000 << "ms" << endl;
			}
		}
	}
	return 0;
...
void ProcessCloudVRPacket(RtpReciver &sess, const RTPSourceData &srcdat, const RTPPacket &rtppack)
{
	...
	{
		pre_viewpoint = cur_viewpoint;//sess.viewpoint;//zsy
		first_flag = true;
		//memcpy(&(sess.viewpoint), rtppack.GetPayloadData(), rtppack.GetPayloadLength());
		memcpy(&(cur_viewpoint), rtppack.GetPayloadData(), rtppack.GetPayloadLength());
	}
}
...
int GetFrame(void* xx) {
	...
	while (1) {
		...
		ThreadSleep(10);
		recv_non_Block(sockfd, buffer, YUV_BUF, 0);
		SDL_SemWait(Lock1);
...
{///ppp
	...
	while (!bQuit)//bQuit = HandleInput();
	{
...
#endif
#ifdef  TCP
			memcpy(client.pic_inbuff , (char*)&viewPointIndex, 2);
			client.send_non_block(client.sockfd,2, 0);
#endif 
			ct2 = 0;
		}
		ct2++;
		...
		if (Mbuff2 != Mbuff1) {		
			...
			test = frame_fifo[Mbuff2].Mbuff;
			viewpoint = frame_fifo[Mbuff2].viewpoint;
			Mbuff2 = (Mbuff2 + 1) % MaxLoadFrameBuffSize;
			if (first && viewpoint==viewPointIndex)
			{
				QueryPerformanceCounter(&nEndTime);
				cout<<"RTT:" << (double)(nEndTime.QuadPart - nBeginTime.QuadPart) / (double)nFreq.QuadPart * 1000 << "ms" << endl;
				first = false;
			}
		}
		SDL_SemPost(Lock1);
		pyramidRot.updateViewPointIndex(viewpoint);
		pyramidRot.getRotationMatrix(mModel);	
#endif
...
int main(int argc, char *argv[])
{
	...
	sess.SetParams(SERVER_IP, PORT_NUMBER, portbase);
#endif
#ifdef TCP
	if (client.client_transfer_init() == -1) {
		printf("Socket error!\n");
		exit(1);
	}
#endif
	...
	client.client_transfer_destroy();
#endif 
